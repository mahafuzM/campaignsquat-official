      ...
      <div className="w-full px-6 md:px-12 py-10">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-12">
          ...
          <div className="bg-white border border-gray-200 rounded-[5px] overflow-hidden shadow-sm">
            <div className="p-8 border-b border-gray-50 bg-[#FCFCFC]">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2 bg-[#F7A400] rounded-[5px] text-black"><LayoutGrid size={24} /></div>
              ...
              <div className="flex flex-wrap md:flex-nowrap gap-4">
                ...
                <button type="button" onClick={handleAddIcon} className="bg-black text-white px-10 py-4 rounded-[5px] font-semibold flex items-center gap-2">
            ...
            <div className="p-8">
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4">
                {availableIcons.map((icon) => (
                  ...
                  >
                    {content.badge === icon.name && <div className="absolute top-0 right-0 p-1.5 bg-[#F7A400] rounded-bl-[5px]"><Check size={12} className="text-white" strokeWidth={4} /></div>}
                    <span className={`text-[13px] font-bold capitalize ${content.badge === icon.name ? "text-[#F7A400]" : "text-black"}`}>{icon.name}</span>
                    <button type="button" onClick={(e) => handleDeleteIcon(icon._id, e)} className="absolute inset-0 bg-red-600/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-[5px]"><Trash2 size={20} className="text-white" /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4">
            <label className="font-semibold text-[14px] uppercase">Main Heading</label>
            <textarea
              rows="2"
          ...
          {/* Description */}
          <div className="flex flex-col gap-4">
            <label className="font-semibold text-[14px] uppercase">Description Paragraph</label>
            <textarea
              rows="4"
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
            <div className="flex flex-col gap-4">
              <label className="font-semibold text-[14px] uppercase">Vimeo Video ID</label>
              <input
                type="text"
            ...
            <div className="flex flex-col gap-4">
              <label className="font-semibold text-[14px] uppercase">Banner Image</label>
              <div className="relative group border-2 border-dashed border-gray-200 rounded-[5px] p-10 hover:border-black transition-all text-center bg-[#FBFBFB]">
                ...
                <div className="flex flex-col items-center gap-2"><span className="text-4xl">📁</span><p className="text-[12px] text-black">Upload or drag and drop image here</p></div>
              </div>
              {previewUrl && <img src={previewUrl} alt="Preview" className="w-full h-[300px] object-cover rounded-xl mt-4 border shadow-md" />}
            </div>
          </div>

          <div className="pt-10 border-t border-gray-200 flex justify-end">
            <button type="submit" disabled={loading} className="bg-[#F7A400] text-black hover:bg-black hover:text-white font-bold py-4 px-12 rounded-xl transition-all shadow-lg active:scale-95 disabled:opacity-50">
