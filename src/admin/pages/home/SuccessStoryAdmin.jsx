...
const SuccessStoryAdmin = () => {
  ...
  const deleteStory = async (index) => {
    if (window.confirm("Are you sure you want to delete this story?")) {
      ...
      try {
        const res = await axios.post(
          "https://api.campaignsquat.com/api/success-story/update",
          updatedData,
        );
        if (res.data) {
          setHistory(res.data.history || []);
    ...
    <div className="p-4 md:p-8 bg-gray-100 min-h-screen font-poppins text-black">
      <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  value={formData.sectionBg}
                  onChange={(e) =>
                    setFormData({ ...formData, sectionBg: e.target.value })
                  }
                />
                <input
                  className="border p-2 flex-grow font-mono"
                  value={formData.sectionBg}
                  readOnly
                />
              </div>
        ...
        <div className="space-y-4">
                        ...
                        <img
                          ...
                          src={
                            story.image.startsWith("http")
                              ? story.image
                              : `https://api.campaignsquat.com${story.image.startsWith("/") ? "" : "/"}${story.image}`
                          }
          ...
          <button
            onClick={addStory}
            className="w-full border-2 border-dashed border-gray-300 py-4 rounded-[5px] flex items-center justify-center gap-2 text-gray-500 hover:border-black hover:text-black transition-all bg-white font-bold  text-sm"
          >
        ...
        <button
          onClick={handleSync}
          disabled={loading}
          className="w-full justify-center bg-[#F7A400] text-black text-[14px] md:text-[15px] hover:text-white hover:bg-[#02050A] font-semibold py-2 rounded-[5px] border-2 border-[#F7A400] transition-all duration-300 flex items-center gap-2"
        >
          <Save size={24} />{" "}
        ...
        <div className="bg-white p-6 rounded-[5px] border border-gray-200 shadow-sm">
          <h3 className="text-xs font-black  border-b pb-2 mb-4">
            Activity History
          </h3>
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {history
              ...
              .map((log, i) => (
                <div
                  key={i}
                  className="flex justify-between text-[11px] bg-gray-50 p-2 rounded border border-gray-100"
                >
