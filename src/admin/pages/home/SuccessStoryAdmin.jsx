...
const SuccessStoryAdmin = () => {
  ...
  const addStory = () => {
    setFormData({
      ...
      stories: [
        ...formData.stories,
        { name: "", role: "", company: "", feedback: "", image: "", videoUrl: "", rating: 5 },
      ],
    });
  };
    ...
    <div className="p-4 md:p-8 bg-gray-100 min-h-screen font-poppins text-black">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-6">
            {formData.stories.map((story, index) => (
              <div key={index} className="bg-white p-6 rounded-[5px] border border-gray-200 shadow-sm relative group">
                <button onClick={() => deleteStory(index)} className="absolute top-4 right-4 text-red-500 hover:scale-110">
                  <Trash2 size={20} />
                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input placeholder="Name" className="border p-2 rounded text-sm" value={story.name} onChange={(e) => updateField(index, "name", e.target.value)} />
                    <input placeholder="Role" className="border p-2 rounded text-sm" value={story.role} onChange={(e) => updateField(index, "role", e.target.value)} />
                    <input placeholder="Company" className="border p-2 rounded text-sm" value={story.company} onChange={(e) => updateField(index, "company", e.target.value)} />
                    <input placeholder="Video URL" className="border p-2 rounded text-sm" value={story.videoUrl} onChange={(e) => updateField(index, "videoUrl", e.target.value)} />
                    <div className="md:col-span-2">
                      <textarea placeholder="Feedback" className="border p-2 rounded text-sm w-full h-20" value={story.feedback} onChange={(e) => updateField(index, "feedback", e.target.value)} />
                    </div>
                    <div className="flex items-center gap-4">
                      <label className="text-[10px] font-bold text-gray-400">Rating (1-5)</label>
                      <input type="number" min="1" max="5" className="border p-1 w-16 text-center" value={story.rating} onChange={(e) => updateField(index, "rating", parseInt(e.target.value) || 5)} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <button onClick={addStory} className="w-full border-2 border-dashed border-gray-300 py-4 rounded-[5px] flex items-center justify-center gap-2 text-gray-500 hover:border-black hover:text-black bg-white font-bold text-sm">
            <Plus size={20} /> Add New Success Story
          </button>
        </div>

        <button onClick={handleSync} disabled={loading} className="w-full justify-center bg-[#F7A400] text-black font-semibold py-2 rounded-[5px] border-2 border-[#F7A400] hover:bg-black hover:text-white transition-all flex items-center gap-2">
