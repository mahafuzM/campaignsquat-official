  ...
  if (!blog) {
    return (
      <div className="min-h-screen bg-[#02050A] text-white flex flex-col items-center justify-center font-poppins">
        ...
        <button onClick={() => navigate("/blog")} className="underline">
          Go Back to Blogs
        </button>
      </div>
    );
      ...
      <div className="max-w-[1445px] mx-auto px-4 md:px-12 lg:px-20 py-8 md:py-18">
        <div className="bg-[#0a0a0a] border border-[#ffffff10] rounded-[5px] overflow-hidden shadow-2xl">
          ...
          <div className="bg-[#02050A] border-y border-[#ffffff10] px-6 md:px-24 py-5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className=" font-semibold text-[12px] md:text-[14px] tracking-widest">
                Share Post
              </span>
            </div>
              ...
              blog.sections.map((item, index) => {
                if (item.type === "heading") {
                  return (
                    <h2
                      key={index}
                      className="text-[20px] md:text-[22px] font-bold text-white mt-4 border-l-4 border-[#f7a400] pl-4"
                    >
                      {item.value}
                    </h2>
                  );
                  ...
                  return (
                    <ul
                      key={index}
                      className="space-y-2 md:space-y-3 ml-2 md:ml-4"
                    >
                      {item.items &&
                        item.items.map((point, i) => {
                          ...
                          return (
                            <li
                              key={i}
                              className="flex items-start gap-3 text-[14px] md:text-[16px] leading-relaxed"
                            >
                              <span className="text-[#f7a400] mt-1.5 flex-shrink-0">
                                •
                              </span>
                              <span>
                                {hasColon ? (
                  ...
                  return (
                    <p
                      key={index}
                      className="text-[14px] md:text-[16px] leading-[1.8] text-justify font-light"
                    >
                      {item.value}
                    </p>
                  );
              ...
              <div className="flex flex-wrap gap-4">
                {blog.tags &&
                  blog.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-6 py-2 bg-[#111] border border-[#ffffff10] text-white text-[10px] md:text-[12px] rounded hover:border-[#f7a400] transition-all cursor-pointer"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="flex justify-center mt-12 mb-16">
          <button
            ...
            className="flex items-center gap-3 bg-[#f7a400] text-black px-12 py-2 rounded-[5px] font-semibold border-2 border-[#f7a400] hover:bg-transparent hover:text-white transition-all duration-300 text-[14px] md:text-[15px]"
          >
