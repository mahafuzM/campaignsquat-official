...
const BlogDetails = () => {
  ...
  const shareOnSocial = (platform) => {
    let link = "";
    if (platform === 'fb') link = `https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`;
    if (platform === 'tw') link = `https://twitter.com/intent/tweet?url=${shareUrl}`;
    if (platform === 'in') link = `https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl}`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#02050A] flex flex-col items-center justify-center">
        ...
        <p className="text-[#F7A400] text-[10px] font-black uppercase tracking-[4px] animate-pulse">Loading Strategy...</p>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="min-h-screen bg-[#02050A] text-white flex flex-col items-center justify-center font-['Poppins'] p-6">
        <h2 className="text-4xl font-black  mb-4 uppercase text-[#F7A400]">Insight Not Found</h2>
        <p className="text-gray-500 mb-8 text-center max-w-md">The strategic article you are looking for might have been moved or archived.</p>
        <button 
  ...
  const parsedSections = Array.isArray(blog.sections) 
    ? blog.sections 
    : (typeof blog.sections === 'string' ? JSON.parse(blog.sections) : []);
        ...
        <div className="bg-[#0a0a0a] border border-[#ffffff10] rounded-[5px] overflow-hidden shadow-2xl">
          ...
          <div className="p-6 md:p-16 lg:p-20 text-center flex flex-col gap-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 border-b border-[#ffffff10] pb-8 text-white">
              ...
              <div className="flex items-center gap-2">
                <Eye size={18} className="text-white" /> {blog.views || "0"} Views
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#02050A] border-y border-[#ffffff10] px-6 md:px-24 py-5 flex items-center justify-between">
            ...
            <div className="flex gap-4">
              <button onClick={() => shareOnSocial('fb')} className="w-10 h-10 border border-[#ffffff15] rounded-full flex items-center justify-center hover:text-[#f7a400] transition-all"><Facebook size={18} /></button>
              <button onClick={() => shareOnSocial('tw')} className="w-10 h-10 border border-[#ffffff15] rounded-full flex items-center justify-center hover:text-[#f7a400] transition-all"><Twitter size={18} /></button>
              <button onClick={() => shareOnSocial('in')} className="w-10 h-10 border border-[#ffffff15] rounded-full flex items-center justify-center hover:text-[#f7a400] transition-all"><Linkedin size={18} /></button>
              <a href="#" className="w-10 h-10 border border-[#ffffff15] rounded-full flex items-center justify-center hover:text-[#f7a400] transition-all"><Instagram size={18} /></a>
            </div>
          </div>
          ...
          <div className="p-6 md:p-16 lg:p-24 flex flex-col gap-10">
            ...
            {parsedSections && parsedSections.map((item, index) => {
              if (item.type === "heading") {
                return (
                  <h2 key={index} className="text-[20px] md:text-[24px] font-bold text-white mt-4 border-l-4 border-[#f7a400] pl-4 ">
                    {item.value}
                  </h2>
                );
                ...
                return (
                  <ul key={index} className="space-y-4 ml-2 md:ml-4 bg-[#ffffff05] p-6 rounded-[5px] border border-[#ffffff10]">
                    {item.items && item.items.map((point, i) => {
                      const hasColon = point.includes(": ");
                      const parts = hasColon ? point.split(": ") : [point];
                      return (
                        <li key={i} className="flex items-start gap-4 text-[14px] md:text-[16px] leading-relaxed">
                          <span className="text-[#f7a400] mt-1.5 flex-shrink-0 text-xl">•</span>
                          <span>
                            {hasColon ? (
                              <><strong className="text-[#f7a400] uppercase text-[13px] tracking-wider">{parts[0]}:</strong> {parts.slice(1).join(": ")}</>
                            ) : (point)}
                          </span>
                        </li>
                      );
                ...
                return (
                  <p key={index} className="text-[15px] md:text-[17px] leading-[1.9] text-white text-justify font-light whitespace-pre-line">
                    {item.value}
                  </p>
                );
              }
            ...
            {blog.tags && blog.tags.length > 0 && (
              <div className="mt-12 pt-8 border-t border-[#ffffff10]">
                <div className="flex flex-wrap gap-4">
                  {blog.tags.map((tag) => (
                    <span key={tag} className="px-6 py-2 bg-[#111] border border-[#ffffff10] text-white text-[10px] md:text-[12px] rounded hover:border-[#f7a400] hover:text-[#f7a400] transition-all cursor-pointer uppercase font-bold tracking-widest">
                      # {tag}
                    </span>
                  ))}
                </div>
